...
os.environ["ENSIGN_CLIENT_ID"] = "YOUR CLIENT ID"
...
class WeatherPublisher:
    ...
    def __init__(self, topic="Daniel_Weather_Test_1", interval=60, locations=LOCS, ):
        """
    ...
    def unpack_noaa_response(self, message):
        ...
        """
        colNames = ['name', 'summary', 'temperature', 'units', 'daytime', 'start', 'end']
        ...
        print(message['geometry']['coordinates'][0][0][0])
        if periods is None:
            raise Exception("unexpected response from forecast request, no periods")
        for period in periods:
            # fields from the NOAA API response that are of use for our modeling:
            
            data = {
                "location":message['geometry']['coordinates'][0][0][0],
                "name": period.get("name", None),
                "summary": period.get("shortForecast", None),
                "temperature": period.get("temperature", None),
                "wind speed": period.get("windSpeed", None),
                "precipitation prob": period['probabilityOfPrecipitation'].get('value', None),
                "humidity":period['relativeHumidity'].get('value', None),
                "detailed forecast": period.get('detailedForecast', None),
